...
bool BATT_readVoltage(bool start, int16_t *v)
{
    ...
    if (start)
    {
        ...
        ADC1_SoftwareTriggerEnable();
        BATT_conversionCnt = 1;
    }
    
    if (BATT_conversionCnt >= 1000)
    {
        ADC1_SoftwareTriggerDisable();
        while(!ADC1_IsConversionComplete(ADC_BATT_CHANNEL));
        *v = ADC1_ConversionResultGet(ADC_BATT_CHANNEL);
        ADC1_Disable(); 
        BATT_conversionCnt = 0;
        done = true;  
    }
    else if (BATT_conversionCnt > 0)
    {
        BATT_conversionCnt++;
    }
    return done;
...
static int16_t BATT_battVoltageCurr =  0;
static int16_t BATT_battVoltagePrev =  0;
static bool BATT_battMonitor = false;

void BATT_handleBatteryVoltage(uint16_t pollingLoopCnt)
{
    bool adcStart = (pollingLoopCnt == 1);
    bool adcDone = BATT_readVoltage(adcStart, &BATT_battVoltageCurr);      
    if (adcDone)
    {
        // USB_PrintLinePrompt("ADC done");
        bool battChanged = (BATT_battVoltageCurr != BATT_battVoltagePrev);
        if (battChanged)
        {
            ...
            if (BATT_battVoltageCurr > battVoltageMin)
            {
                battVoltageDeltaActual = BATT_battVoltageCurr - battVoltageMin;
            }
...
//            sprintf(batteryField, "%4d%% ", percent);
            sprintf(batteryField, "%4d%% ", BATT_battVoltageCurr);
            bool rising = BATT_battVoltageCurr > BATT_battVoltagePrev;
            BATT_battVoltagePrev = BATT_battVoltageCurr;
            if (BATT_battMonitor)
            {
                USB_printfLinePrompt("Battery %s to %d", 
                                     rising ? "rising" : "falling",
                                     BATT_battVoltageCurr);
            }
        }
    }
}
